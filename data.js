...
FAQ['word-to-pdf']=[
  ["Is this a real PDF, or does it just open the print dialog?","A real PDF, generated programmatically in your browser and downloaded automatically \u2014 there is no print dialog anywhere in this tool. The document is rendered using the same fonts, headings, bold/italic, lists, tables and spacing as the original, then written into an actual PDF file page by page."],
  ["Will the text be selectable/searchable in the PDF?","Not currently \u2014 each page is rendered as a high-quality image and embedded in the PDF, which preserves visual formatting very faithfully but means the text isn't selectable or searchable, and the file is somewhat larger than a text-based PDF. This trade-off exists because faithfully reproducing arbitrary real-world Word formatting (custom fonts, complex tables, mixed layouts) is far more reliable by rendering exactly what the browser displays than by hand-reconstructing every element as native PDF text."],
  ["How are page breaks decided?","The tool measures every paragraph, heading, list item and table row before generating the PDF, and only breaks a page at one of those boundaries \u2014 never through the middle of a paragraph or table row. It does not yet read explicit \u201cinsert page break\u201d marks from the original Word document; very long single elements (e.g. one huge image) may still span awkwardly if no natural break exists nearby."],
  ["What if my document uses custom named styles?","Standard styles (Heading 1\u20134, Title, Subtitle, Quote, Strong, Emphasis) are mapped to matching PDF formatting. Highly customised or unusual Word template styles may render as plain paragraphs if Word uses non-standard style names."],
  ["Are my files uploaded anywhere?","No \u2014 reading the DOCX, converting it, and generating the PDF all happen locally in your browser. Nothing is sent to a server."]
];
FAQ['pdf-to-excel']=[
  ...
  ["What output formats are available?","Excel (.xlsx) with formatting preserved, Excel (.xlsx) with data only for a simpler file, or plain CSV. All three come from the same table-reconstruction pass \u2014 the choice only affects how the result is packaged, not how tables are detected."],
  ["How does document type detection work?","It's rule-based structural analysis, not a trained AI model \u2014 the tool measures real signals (how short the page is, whether a large title dominates, whether column positions genuinely repeat across many rows) and reports a plain High/Medium/Low confidence with the actual reasons, rather than an invented precise percentage. It reliably tells short documents, table-structured documents, and long-form prose apart; it doesn't attempt to guess a specific document genre (invoice vs. price list vs. bank statement all share the same underlying grid structure)."],
  ["What's the difference between the reconstruction modes?","Smart Auto detects each page's structure and reconstructs it accordingly \u2014 recommended for most PDFs. Preserve Visual Layout always groups content into readable blocks instead of a grid, which suits business cards, resumes and other non-tabular documents. Editable Spreadsheet always uses the table-grid path, best when you plan to edit or run formulas on the result. Tables Only extracts exclusively what was classified as tabular content, skipping surrounding text. Text Only is a simple reading-order dump with no grid at all."],
  ["Can it embed logos or images into the spreadsheet?","Not currently \u2014 the underlying spreadsheet library the tool uses doesn't support writing images into cells. Images in the source PDF are skipped rather than silently dropped without mention; this is a real, known limitation, not an oversight."]
];
FAQ['ai-image-upscaler']=[
 ['How does the AI upscaling work?','The default engine runs an ESRGAN super-resolution model directly in your browser (downloaded once from a CDN, then cached). For large photos where full-resolution neural inference would take minutes, the tool downscales to a size the model can process quickly, runs genuine AI detail recovery on that, then resizes to your exact target dimensions \u2014 real neural enhancement either way, not skipped for being large. If your browser can\u2019t run the model at all, the tool falls back to a multi-pass reconstruction (noise cleanup, progressive resizing, local-contrast recovery, edge-aware sharpening) \u2014 real image science, clearly labeled as non-AI, never presented as the neural engine.'],
...
FAQ['ai-photo-enhancer']=[["How does the enhancement work?","The tool analyzes your photo's histogram, color balance, noise and sharpness on your device, then applies targeted corrections: white balance, exposure, contrast, shadow/highlight recovery, vibrance, edge-safe noise reduction, clarity and gated sharpening \u2014 plus a baseline color and clarity lift even on photos that already measure well, since a statistically \"fine\" photo can still look flat next to nothing. It is honest image science running in your browser \u2014 and it always labels which engine produced your result."],["Is my photo uploaded anywhere?","No. The default engine processes everything on your device. An optional cloud engine for AI face restoration can be enabled by the site, and the tool will clearly say so whenever it is used."],["Does it enhance faces?","The Portrait preset includes a skin-tone-aware smoothing pass \u2014 pixels in a typical skin-tone color range get light blemish softening, while eyes, hair and everything else are left untouched (and get the tool's normal sharpening), which is the same colour-range technique used as a first pass in a lot of photo apps. This is a colour heuristic, not face detection \u2014 it can't specifically locate or sharpen an eye. Real facial reconstruction (Remini-class) needs a server GPU model, available via the optional cloud engine."],["Can I control how strong the enhancement is?","Yes \u2014 Subtle, Balanced, Strong or Maximum, applied on top of whichever preset is selected. Corrective adjustments (fixing an actually-too-dark exposure or a color cast) aren't scaled by this, only the stylistic ones (contrast, vibrance, clarity, sharpness) \u2014 so Subtle won't leave a genuinely underexposed photo still dark."],["Does it preserve transparency?","Yes \u2014 the alpha channel is never modified. PNG and WEBP keep transparency; JPG flattens onto white."],["What are the presets?","Auto applies corrections derived from the analysis of your specific photo. Portrait, Landscape, Night, Document, Product, Old Photo and Anime are tuned starting points \u2014 adjusting any slider switches to Custom."]];
FAQ['ai-object-remover']=[["How does the removal work?","You paint a mask over unwanted objects; the engine reconstructs the hidden background from the surrounding pixels using content-aware fill \u2014 multiscale reconstruction, edge detection that reconnects interrupted lines (fences, railings, horizons) through the removed area, patch-based texture matching, and an automatic refinement pass if the first fill looks too smooth for its surroundings. The same family of techniques behind Photoshop's original Content-Aware Fill. It runs entirely on your device and always labels which engine produced your result."],["What does it remove well?","Power lines, trash, text, logos, people and objects on textured or repeating backgrounds \u2014 sky, grass, walls, water, roads, sand. Very large removals across complex structure (a person covering detailed architecture) are harder for on-device reconstruction; an optional cloud AI inpainting engine can be enabled by the site for those cases, and the tool will say clearly when it's used."],["Can I remove multiple objects?","Yes \u2014 paint several areas and remove them in one pass, or use \u201cRemove more\u201d to keep cleaning up the result iteratively."],["Is my photo uploaded anywhere?","No \u2014 the default engine processes everything in your browser. Nothing leaves your device."],["Can I remove watermarks?","Only from images you own or have rights to edit. Removing watermarks from other people's work violates copyright \u2014 please respect creators."]];
