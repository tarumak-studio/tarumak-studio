...
function dropzone(el,input,onFiles){if(!el||!input)return;el.addEventListener('click',()=>input.click());el.addEventListener('keydown',ev=>{if(ev.key==='Enter'||ev.key===' '){ev.preventDefault();input.click();}});['dragover','dragenter'].forEach(e=>el.addEventListener(e,ev=>{ev.preventDefault();el.classList.add('drag');}));['dragleave','drop'].forEach(e=>el.addEventListener(e,ev=>{ev.preventDefault();el.classList.remove('drag');}));el.addEventListener('drop',ev=>onFiles(ev.dataTransfer.files));input.addEventListener('change',()=>onFiles(input.files));}
